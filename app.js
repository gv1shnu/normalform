...
function renderMoveCue(moves) {
  return `<div class="transform-cue" aria-label="Where each value goes"><span class="transform-cue-label">Columns moved</span>${moves.map(({ from, to }) => `<div class="move-item"><code>${esc(from)}</code><span aria-hidden="true">→</span><strong>${esc(to)}</strong></div>`).join("")}</div>`;
}

function renderSplitSummary(split) {
  if (!split) return "";
  const into = split.into.map((name) => `<span class="split-into">${esc(name)}</span>`).join(`<span class="split-plus" aria-hidden="true">+</span>`);
  return `<div class="split-summary" aria-label="Table split for this step"><span class="split-summary-label">Table split</span><div class="split-summary-flow"><span class="split-from">${esc(split.from)}</span><span class="split-arrow" aria-hidden="true">→</span><span class="split-targets">${into}</span></div></div>`;
}
...
function renderStage(index) {
  ...
  const transforms = [
    null,
    { bad:{ Campus_Workbook:[3,4,5,6] }, changed:["Campus_Record_1NF","Student_Hobby_Language","Student_Club_Event"], split:{ from:"Campus_Workbook", into:["Campus_Record_1NF","Student_Hobby_Language","Student_Club_Event"] }, wrong:"Some cells hold a whole list of values. When many facts share one cell, you can't reliably find, update, or check any single one of them.", dependency:"One StudentID → many courses, hobbies, languages, and events", fix:"Give every value its own row and cell, so each row states one plain fact. The messy lists become separate tables.", moves:[{from:"Courses + grade + tutor",to:"Campus_Record_1NF"},{from:"Hobbies + Languages",to:"Student_Hobby_Language"},{from:"Club events",to:"Student_Club_Event"}] },
    { bad:{ Campus_Record_1NF:[1,2,3,5] }, changed:["Student_Profile_2NF","Course","Enrollment_2NF"], split:{ from:"Campus_Record_1NF", into:["Student_Profile_2NF","Course","Enrollment_2NF"] }, wrong:"The key here is StudentID and CourseID together. But a student's name depends on StudentID alone, and a course's name on CourseID alone — so those details repeat on every row.", dependency:"StudentID → StudentName, DeptID · CourseID → CourseName", fix:"Split off the parts that depend on only half the key: student details into one table, course details into another.", moves:[{from:"StudentName, DeptID, DeptName",to:"Student_Profile_2NF"},{from:"CourseName",to:"Course"},{from:"Grade, TutorID",to:"Enrollment_2NF"}] },
    { bad:{ Student_Profile_2NF:[3] }, changed:["Student","Department"], split:{ from:"Student_Profile_2NF", into:["Student","Department"] }, wrong:"A department's name really belongs to the department, not the student. So it gets repeated for every student in the same department.", dependency:"StudentID → DeptID → DeptName", fix:"Give departments their own table, and point each student at it by ID.", moves:[{from:"StudentName, DeptID",to:"Student"},{from:"DeptName",to:"Department"}] },
    { bad:{ Enrollment_2NF:[1,3] }, changed:["Tutor_Course","Student_Tutor_Grade"], split:{ from:"Enrollment_2NF", into:["Tutor_Course","Student_Tutor_Grade"] }, wrong:"Each tutor teaches one course, so the tutor decides the course. But the tutor isn't this table's key, so the course repeats for every student that tutor helps.", dependency:"TutorID → CourseID, but TutorID isn't the key", fix:"Store each tutor's course once in its own table, and keep only student, tutor, and grade together.", moves:[{from:"TutorID, CourseID",to:"Tutor_Course"},{from:"StudentID, TutorID, Grade",to:"Student_Tutor_Grade"}], extra:"Trade-off: after this split, no single table can enforce that a student takes each course with just one tutor." },
    { bad:{ Student_Hobby_Language:[1,2] }, changed:["Student_Hobby","Student_Language"], split:{ from:"Student_Hobby_Language", into:["Student_Hobby","Student_Language"] }, wrong:"Hobbies and languages have nothing to do with each other. Keeping them in one table pairs every hobby with every language — rows that don't mean anything.", dependency:"StudentID ↠ Hobby · StudentID ↠ Language", fix:"Give hobbies and languages their own separate tables, one fact each.", moves:[{from:"Hobby",to:"Student_Hobby"},{from:"Language",to:"Student_Language"}] },
    { bad:{ Student_Club_Event:[0,1,2] }, changed:["Student_Club","Student_Event","Club_Event"], split:{ from:"Student_Club_Event", into:["Student_Club","Student_Event","Club_Event"] }, wrong:"Two of the pairings aren't enough. Joining just student-club with student-event invents club-at-event combinations that never actually happened.", dependency:"join {Student_Club, Student_Event, Club_Event} = Student_Club_Event", fix:"Keep all three pair tables. Only joining all three together rebuilds the real data without inventing extra rows.", moves:[{from:"StudentID + Club",to:"Student_Club"},{from:"StudentID + Event",to:"Student_Event"},{from:"Club + Event",to:"Club_Event"}] }
  ];
  ...
  return stage(index, FORMS[index], rules[index], `${explain(config)}${renderSplitSummary(config.split)}${split}${contrast}${tradeoff}${handoff}${ending}`);
}
